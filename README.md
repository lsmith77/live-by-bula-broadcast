...
| Identity fields | Nickname, pronouns and name pronunciation as structured fields, self-declared through the CSV or corrected at the desk, shown beside the name on the roster, the player sheet and the field view. Less common pronoun sets are emphasised — those are the ones a reading habit gets wrong — and the full line rides on hover on the field view. A prep-time review maps variant spellings onto the two common sets, one click each, never automatically |
