...
- `docs/SETUP.md` — a concept, nothing built: configurable per-rig setup profiles and pre-game checks. The idea worth keeping is that a checklist earns its place only where the software can *verify* an item or where getting it wrong puts something false on air; ten such items are already machine-checkable and surfaced nowhere.
