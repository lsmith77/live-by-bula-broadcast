...
- `docs/RELAY.md` — a concept, nothing built: whether the state could live in the browsers with a server that only relays. Worth reading for two things — that the existing stores are already nearly CRDTs, and the two-tier rule about which surfaces may take dependencies.
