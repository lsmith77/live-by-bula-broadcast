...
**Normalising is prep work, done by hand.** The emphasis rule needs the two common sets in a canonical form, but the CSV deliberately stays free text — a dropdown in a team-visible sheet would make the two common answers a form and everything else a disclosure, which is the singling-out that actually matters. The reconciliation happens at the desk instead: each roster panel lists every declared set the rule does not recognise ("He", "she/her/hers", another language), with one-click mapping onto `he/him` or `she/her`. A variant that plainly means a common set gets mapped; anything genuinely declared is left as the player wrote it, emphasised. Nothing is ever mapped automatically — the click is a person's judgement, and when in doubt the answer is to leave it.
